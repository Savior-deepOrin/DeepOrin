# deepOrin Engineering Spec

> deepOrin is a coding agent: a thin harness driving multiple models, with **all
...
```
deeporin/
├── go.mod / go.sum          # module deeporin; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── deeporin.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/deeporin/main.go          # entry; blank-imports built-in providers/tools
├── cmd/deeporin-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .deeporin/commands/*.md
...
- `cmd/deeporin-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. deepOrin manages this with
...
- The dropped originals are archived under the user config dir
  (`deeporin/archive/<timestamp>.jsonl`; see §5 for its per-OS location), one
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  non-interactive run
  (`deeporin run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
Out of the box (`mode = "ask"`, no rules) `deeporin run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `deeporin chat` now prompts before
...
- **Custom commands** are Markdown files under `.deeporin/commands/` (project) and
  `deeporin/commands/` in your OS config dir (user; see §5); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — deepOrin
...
Resolution order: **flag > project `./deeporin.toml` > the user config file
> built-in defaults**. The user config lives in your OS config dir — `~/.config/deeporin/`
...
`deeporin.toml` should override them only when the repository needs shared
runtime bounds.

```toml
default_model = "deepseek"   # provider name (→ its default model) or "provider/model"
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $DEEPORIN_LANG

[agent]
system_prompt = "You are deepOrin, a coding agent..."  # or system_prompt_file = "..."
...
command = "deeporin-plugin-example"
args    = []
# env   = { FOO = "bar" }
...
`deeporin setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `deeporin.toml` wins (it is the more explicit,
deepOrin-specific source). This lets a server already configured for Claude work in
deepOrin unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o deeporin ./cmd/deeporin`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `deeporin run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `deeporin run`.
