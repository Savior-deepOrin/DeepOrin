...
How deepOrin ships, who can ship what, and the canary-before-stable flow.
...
|---|---|---|
| npm | `latest` (0.x), `next` (1.x) | `canary` (`npm i deeporin@canary`) |
...
3. **Test** — testers install `deeporin@canary` (CLI) or grab the desktop canary
...
6. **Promote to default install** (optional, when 1.x should become the bare `npm i` target):
   ```sh
   npm dist-tag add deeporin@1.4.0 latest
   ```
