...
`agent.reasoning_language` 控制模型服务暴露“可见思考过程”时，deepOrin 希望它优先使用哪种语言。
...
桌面端设置会写入用户级默认值。项目仍然可以通过 `./deeporin.toml` 覆盖。
...
```bash
deeporin config reasoning-language auto
deeporin config reasoning-language zh
deeporin config reasoning-language en
```

默认写入用户配置。要写入当前项目的覆盖配置：

```bash
deeporin config reasoning-language --local zh
```

在 `deeporin chat` 内可以用斜杠命令：
...
斜杠命令会写入用户级设置，并立即更新当前 chat controller，后续 turn 生效。它不会改写当前项目的 `deeporin.toml`；如果要写项目级覆盖，请使用带 `--local` 的 shell 命令。
...
```text
./deeporin.toml > 用户 config.toml > 内置默认值
```
...
当设置为 `zh` 或 `en` 时，deepOrin 会把一个很小的 `<reasoning-language>` 临时 block 放进本次 user turn。它不会改变：
