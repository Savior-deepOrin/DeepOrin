...
> bots, how to use deepOrin from IM, and how approvals, Ask questions, YOLO, and
> bot commands work.
...
After a bot is connected, you can send deepOrin messages from Feishu, Lark, or
...
- Ask deepOrin to inspect code, read docs, explain errors, or summarize findings.
...
Open the deepOrin desktop app and go to **Settings -> Bots**. In **Add IM Bot**,
...
sequenceDiagram
  ...
  participant R as "deepOrin desktop"
...
deepOrin bots use the same permission system as the desktop app. Ask mode is the
...
No. A normal deepOrin app upgrade or overwrite install does not require
rebinding.

Bindings are stored in the user's deepOrin data, not inside the app bundle:

- Bot connections, remote IDs, allowlists, model choices, and approval modes are
  stored in the user config.
- Feishu and Lark secrets are stored in the deepOrin user credentials file.
- The WeChat scanned account token is stored in the deepOrin user data
  directory.

You may need to bind again if:

- The deepOrin user config directory was deleted.
...
| Connected but no reply | Make sure the deepOrin desktop app is running, the bot connection is enabled, and the sender ID is allowlisted or access is open. |
