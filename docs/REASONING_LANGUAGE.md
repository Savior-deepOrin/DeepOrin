...
Inside `deeporin chat`, use the slash command:
...
`deeporin.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
deeporin run "explain this module"
```
...
```text
./deeporin.toml > user config.toml > built-in defaults
```
...
When set to `zh` or `en`, deepOrin adds a small transient
