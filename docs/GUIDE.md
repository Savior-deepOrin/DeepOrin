...
Resolution order: **flag > `./deeporin.toml` > the user config file >
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $DEEPORIN_LANG
...
`deeporin chat` can grant Bash as an exact command or as a conservative command
...
`deeporin run` stays autonomous but still honours `deny`.
...
deepOrin is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
exposes. `make build` also produces `bin/deeporin-plugin-example` — a runnable
...
**Already have an `.mcp.json`?** Drop it in the project root and deepOrin
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `deeporin.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.deeporin/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `deeporin.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In `deeporin chat`, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.deeporin/commands/`
(project) or `~/.config/deeporin/commands/` (user) — `review.md` becomes
...
`/memory` lists both memory documents (`DEEPORIN.md` / `AGENTS.md`) and saved
...
Embed `@` references in a message and deepOrin resolves them before sending, as
...
`deeporin setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `DEEPORIN.md` / `AGENTS.md` memory and a small read-only
...
`./deeporin.toml` only when that repository needs a shared override, such as a
...
automatically: deepOrin first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` in `deeporin chat` to change the user-level setting, or
`deeporin config auto-plan off|on` from a shell/script. The visible reasoning
language uses the same shape: `/reasoning-language auto|zh|en` in chat, or
`deeporin config reasoning-language auto|zh|en` in a shell/script. Pass
