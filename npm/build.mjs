...
for (const t of TARGETS) {
  const name = `@deeporin/cli-${t.node}`;
  const dir = join(STAGE, `cli-${t.node}`);
  const exe = t.goos === "windows" ? "deeporin.exe" : "deeporin";
    ...
    `${JSON.stringify(
      {
        name,
        version,
        description: `deeporin prebuilt binary for ${t.node}.`,
        ...
        repository: {
          type: "git",
          url: "git+https://github.com/Savior-deepOrin/deeporin.git",
        },
      },
      null,
      2,
...
const mainDir = join(STAGE, "deeporin");
mkdirSync(mainDir, { recursive: true });
cpSync(join(HERE, "deeporin", "bin"), join(mainDir, "bin"), { recursive: true });
cpSync(join(ROOT, "README.md"), join(mainDir, "README.md"));

const mainPkg = JSON.parse(
  readFileSync(join(HERE, "deeporin", "package.json"), "utf8"),
);
...
// stable to default with a manual `npm dist-tag add deeporin@<ver> latest`.
...
console.log(`publish deeporin@${version} (${distTag})`);
