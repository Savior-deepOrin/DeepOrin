...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "deeporin.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 deepOrin e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "deeporin/internal/agent/foo.go" into
