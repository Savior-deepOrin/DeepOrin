...
#   macOS:   deepOrin-darwin-<arch>.zip                  (ditto archive; updater channel)
#            deepOrin-darwin-universal.dmg               (drag-to-install; human download)
#   Windows: deepOrin-windows-<arch>-installer.exe       (NSIS per-user installer; updater channel)
#            deepOrin-windows-<arch>.zip                 (portable human download)
#   Linux:   deepOrin-linux-<arch>.tar.gz                (bare binary; updater channel)
#            deepOrin-linux-<arch>.deb                   (Debian/Ubuntu package; human download)
#
...
APPNAME="deepOrin"            # wails.json productName -> deepOrin.app
BINNAME="deeporin-desktop"    # wails.json outputfilename -> linux binary name
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (deeporin-desktop.app); repackage
	# it as deepOrin.app for a clean user-facing name.
	staging=$(mktemp -d)
	app="$staging/${APPNAME}.app"
	cp -R "build/bin/deeporin-desktop.app" "$app"
