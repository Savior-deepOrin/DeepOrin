...
<header><a class="brand" href="https://deeporin.local">${LOGO}</a><a class="brand" href="https://deeporin.local">deepOrin</a><span class="crumb">/ ${esc(crumb)}</span>${nav ? `<span class="nav">${nav}</span>` : ""}</header>
${body}</div></body></html>`;
}
