...
// headless `deeporin run` path, where the Sink renders to stdout and the caller
...
// out of project deeporin.toml while preserving the CLI AddMCPServer semantics.
...
// DEEPORIN.md by default) — the write side of "#<note>". Returns the file written.
