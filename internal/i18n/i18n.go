...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — deeporin setup
	StepSetKey      string // step 2 command label

	// `deeporin init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // deeporin chat step desc
	StepRunDesc    string // deeporin run step desc
	HelpFooter     string // dim footer linking to deeporin help
	...
	WroteFileFmt          string // "Wrote %s" — used for deeporin.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from deeporin.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: deeporin run [--model NAME] <task>"
	...
	// `deeporin upgrade` / `deeporin update` — self-update
...
//
// Priority: override > DEEPORIN_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"DEEPORIN_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
