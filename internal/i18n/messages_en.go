...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `deeporin chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold deeporin.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `deeporin chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `deeporin setup`.",
	StepSetKeyHint: "run `deeporin setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "deeporin help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `deeporin chat`",
	...
	OutputStyleHint:    "set agent.output_style in deeporin.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in deeporin.toml",
	...
	ArgLanguageAuto: "auto-detect from DEEPORIN_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .deeporin/settings.json (project, after trust) or ~/.deeporin/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in deeporin.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create DEEPORIN.md in the project root",
	...
	MissingKeyIntro:       "deeporin.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `deeporin setup` or export DEEPSEEK_API_KEY=...), then run `deeporin run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from deeporin.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: deeporin run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `deeporin setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `deeporin setup`.",
	...
	UsageBody: `deeporin — a config- and plugin-driven coding agent (multi-model)

Usage:
  deeporin chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  deeporin run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  deeporin serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  deeporin acp [--model NAME]                           serve Agent Client Protocol over stdio (also: deeporin --acp)
  deeporin setup [path]                                 interactive config wizard; writes deeporin.toml (+ .env)
  deeporin config auto-plan [off|on]                    configure automatic plan mode
  deeporin config reasoning-language [auto|zh|en]        configure visible reasoning language
  deeporin mcp <add|remove|list>                        manage MCP servers in deeporin.toml
  deeporin doctor [--json]                              print redacted local diagnostics
  deeporin bot start|doctor|weixin-login                multi-channel IM bot gateway
  deeporin upgrade [--check] [--force]                   self-update to the latest release
  deeporin version
  deeporin help

Examples:
  deeporin chat
  deeporin chat --continue
...
Configuration:
  Resolution: flag > ./deeporin.toml > ~/.config/deeporin/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'deeporin setup' to scaffold a config; see docs/SPEC.md.
`,
}
