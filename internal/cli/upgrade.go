...
// upgradeCommand handles `deeporin upgrade` (and `deeporin update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("deeporin-%s-%s", runtime.GOOS, runtime.GOARCH)
	var asset *ghAsset
...
// "npm-v"). Prereleases are kept: only 1.x carries `deeporin upgrade`, and the
...
func fetchLatestRelease(c *http.Client) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "deeporin-cli")
...
// extractBinary pulls the "deeporin" binary from a .tar.gz or .zip archive.
...
// rename it aside to .deeporin.old first, then place the new binary.
