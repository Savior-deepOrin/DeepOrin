// Package cli implements deeporin's command-line entry: subcommand routing, flag
...
import (
	...
	"deeporin/internal/agent"
	"deeporin/internal/boot"
	"deeporin/internal/builtinmcp"
	"deeporin/internal/config"
	"deeporin/internal/control"
	"deeporin/internal/event"
	"deeporin/internal/i18n"
	"deeporin/internal/notify"
	"deeporin/internal/provider"
	"deeporin/internal/provider/openai"
	"deeporin/internal/serve"
	"time"
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `deeporin init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
...
func runServe(args []string) int {
	...
	fmt.Printf("deeporin serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// Keys always go to the deeporin-owned global credentials file so they never land
...
// project-local deeporin.toml only when the user config dir can't be resolved.
func defaultConfigTarget() string {
	if p := config.UserConfigPath(); p != "" {
		return p
	}
	return "deeporin.toml"
}

// defaultEnvTarget is the deeporin-owned global credentials file, falling back to
...
// resolveSetupTargets picks where `deeporin setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./deeporin.toml
...
// setupConfig runs the configuration wizard (the `deeporin setup` command),
// writing config.toml to the user-global dir (or ./deeporin.toml under --local)
// and API keys to the deeporin-owned global .env — never a project's own .env.
...
// initHint handles `deeporin init`. Unlike a config scaffold, project memory is
...
// (and to `deeporin setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to envPath (the deeporin-owned global .env, never a
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("deeporin")),
		"",
...
// that older versions of the wizard wrote into deeporin.toml. They collide
...
// can warn the user to clean up deeporin.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `deeporin setup`.
// The host-based slug also gives users a meaningful name to grep for in
// deeporin.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing deeporin.toml deal with SKU names
...
// deeporin.toml that defines only [[providers]] for deepseek otherwise hides
...
// appended to the deeporin-owned global .env so the chat session that follows
...
// are appended — so re-running `deeporin setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("deeporin") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("deeporin setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("deeporin chat", i18n.M.StepChatDesc)
	step(`deeporin run "task"`, i18n.M.StepRunDesc)
...
func configAutoPlanCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./deeporin.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  deeporin config auto-plan [--local] [off|on]
  deeporin config reasoning-language [--local] [auto|zh|en]
`)
}
