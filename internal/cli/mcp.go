...
// mcp.go holds the MCP server-management surface shared by the `deeporin mcp`
...
// mcpCommand implements `deeporin mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-16s (built-in, not installed)%s  run `deeporin codegraph install`", "codegraph", codegraphMeta)
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: deeporin mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to deeporin.toml).

Usage:
  deeporin mcp list
  deeporin mcp add <name> <command> [args...]        stdio server
  deeporin mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  deeporin mcp add <name> --sse  <url>               remote (legacy SSE)
  deeporin mcp import                                import Codex-enabled servers from cc-switch
  deeporin mcp remove <name>
...
Examples:
  deeporin mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  deeporin mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
