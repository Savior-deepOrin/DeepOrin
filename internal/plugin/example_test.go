...
// buildExamplePlugin compiles cmd/deeporin-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "deeporin-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "deeporin/cmd/deeporin-plugin-example").CombinedOutput()
	if err != nil {
