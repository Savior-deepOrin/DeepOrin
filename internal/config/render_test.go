...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(got, "deeporin/config.toml") {
		t.Fatalf("display path = %q, want deeporin/config.toml suffix", got)
	}
...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${DEEPORIN_PROXY_PASSWORD}",
		},
	}
	...
	orig.Bot.Connections = []BotConnectionConfig{{
		...
		WorkspaceRoot:    "/tmp/deeporin-bot",
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "deeporin-plugin-example"},
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/deeporin-bot" {
	...
	if len(got.Bot.Connections[0].SessionMappings) != 1 || got.Bot.Connections[0].SessionMappings[0].Scope != "project" || got.Bot.Connections[0].SessionMappings[0].WorkspaceRoot != "/tmp/deeporin-bot" {
