...
// separate from the `deeporin setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to DEEPORIN_LANG / locale.
...
func (c *Config) SetDesktopAppearance(theme, style string) error {
	...
	if normalized == "" {
		return fmt.Errorf("desktop theme style %q: must be graphite|aurora|slate|carbon|nocturne|amber|cyber", style)
	}
...
// .mcp.json-only server would otherwise be serialized into deeporin.toml or the
...
func pluginTOMLSourcePath(name string) string {
	for _, path := range []string{"deeporin.toml", userConfigPath()} {
...
// half-written deeporin.toml that fails to parse on next load. Parent directories
...
func SaveMinimalProjectAutoPlan(path, mode string) (string, error) {
	...
	body := fmt.Sprintf(`# deepOrin project configuration.
...
func writeConfigFile(path, body string) error {
	...
	tmp, err := os.CreateTemp(dir, ".deeporin.*.toml.tmp")
	if err != nil {
...
// (SourcePath), or to ./deeporin.toml when none exists yet — the conventional
...
// SaveForRoot saves the config to root's deeporin.toml, falling back to the
// user's global config when root has no existing deeporin.toml.
