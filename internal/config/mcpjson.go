...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. deepOrin reads
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.deeporin/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".deeporin", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.deeporin/config.json and returns its enabled
...
// already declare. deeporin.toml's [[plugins]] win on a name collision: it is the
// deepOrin-specific, more explicit of the two, so it overrides the shared,
