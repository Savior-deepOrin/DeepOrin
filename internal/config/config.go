// Package config loads deepOrin's runtime configuration from TOML. Resolution order:
// flag > project ./deeporin.toml > user config.toml (in the OS user-config dir) > built-in defaults.
...
// Config is deepOrin's runtime configuration.
type Config struct {
	ConfigVersion int                 `toml:"config_version"`
	DefaultModel  string              `toml:"default_model"`
	Language      string              `toml:"language"` // ui/model language tag (e.g. "zh"); empty = auto-detect from $LANG / $DEEPORIN_LANG
	UI            UIConfig            `toml:"ui"`
...
// lets deeporin fetch the CodeGraph runtime into its cache when CodeGraph is
// enabled but missing; set false to require an explicit `deeporin codegraph
...
// BuiltInMCPConfig controls deepOrin-shipped MCP servers that require no user
...
// the project roots (.deeporin/.agents/.agent/.claude under the workspace) and
...
type AgentConfig struct {
	...
	// .deeporin/output-styles/<name>.md). Empty = the unmodified prompt.
...
// deeporin.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
...
const DefaultSystemPrompt = `You are deepOrin, a coding agent focused on executing code tasks.
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `deeporin run` autonomous (no TTY → ask
		// resolves to allow) while `deeporin chat` prompts before writers. Users add
...
// the v0.x ~/.deeporin/config.json's mcpServers. A .env in the working directory
...
// each project's deeporin.toml + .env + .mcp.json are resolved independently
...
func LoadForRoot(root string) (*Config, error) {
	...
	projectTOML := "deeporin.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "deeporin.toml")
	}
	...
	// project deeporin.toml doesn't drop the global config's MCP servers.
	...
	// deeporin.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// Lowest priority: the v0.x ~/.deeporin/config.json's mcpServers, so upgrading
...
// LoadForEdit returns a config to seed the `deeporin setup` wizard when reconfiguring:
...
func userConfigPath() string {
	...
	return filepath.Join(dir, "deeporin", "config.toml")
}
...
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/deeporin/config.toml"
	}
...
// UserCredentialsPath is the deeporin-owned global secrets file, beside
// config.toml in the user config dir (os.UserConfigDir()/deeporin/credentials). It
...
func UserCredentialsPath() string {
	...
	return filepath.Join(dir, "deeporin", "credentials")
}
...
func ArchiveDir() string {
	...
	return filepath.Join(dir, "deeporin", "archive")
}

// SessionDir is where chat sessions are persisted (one .jsonl per session).
// Used by `deeporin chat --continue` / `--resume` to find the recent ones. Empty
...
func SessionDir() string {
	...
	return filepath.Join(dir, "deeporin", "sessions")
}
...
// existing dirs (UserConfigDir/deeporin/...) so the whole reasonix state tree
...
func CacheDir() string {
	...
	return filepath.Join(dir, "deeporin", "cache")
}

// MemoryUserDir returns the deeporin user config root (…/deeporin), under which
// the user-global DEEPORIN.md and the per-project auto-memory store live. Empty
...
func MemoryUserDir() string {
	...
	return filepath.Join(dir, "deeporin")
}

// ConventionDirs are the parent directories scanned for agent assets (skills,
// commands), in canonical-first order. .deeporin is ours; .agents / .agent /
...
// .deeporin/settings.json (see internal/hook).
var ConventionDirs = []string{".deeporin", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .deeporin ends up the
...
// on a name clash. Order: home-dir convention dirs (~/.claude/commands … ~/.deeporin/commands),
// the legacy XDG user dir (~/.config/deeporin/commands), then the project's
// convention dirs (.claude/commands … .deeporin/commands). Scanning the .claude /
...
func CommandDirsForRoot(root string) []string {
	...
	if dir, err := os.UserConfigDir(); err == nil {
		dirs = append(dirs, filepath.Join(dir, "deeporin", "commands")) // legacy XDG user dir
	}
