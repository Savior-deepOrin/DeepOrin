...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("DEEPORIN_TEST_SECRET", "sk-live-secret")
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Model:     "model-a",
		APIKeyEnv: "DEEPORIN_TEST_SECRET",
	}}
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	...
	if !strings.Contains(text, "deeporin 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config deeporin.toml: parse boom"}})
