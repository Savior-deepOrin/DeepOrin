...
// (.deeporin/settings.json, only when the project is trusted) and a global
// (~/.deeporin/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".deeporin"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.deeporin/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.deeporin/settings.json.
