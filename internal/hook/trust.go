...
// Trust gates project hooks. A project's .deeporin/settings.json can run
...
// root. The trust flag lives in user-global state (~/.deeporin/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.deeporin/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.deeporin.
...
// TrustPath is ~/.deeporin/trust.json (homeDir overrides ~).
