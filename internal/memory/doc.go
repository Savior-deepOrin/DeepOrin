// Package memory implements deepOrin's persistent memory. It mirrors Claude
// Code's two-layer model while honoring deepOrin's cache-first architecture:
//
//   - Hierarchical doc memory: DEEPORIN.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/deeporin/DEEPORIN.md
	ScopeAncestor Scope = "ancestor" // a DEEPORIN.md above the project root
	ScopeProject  Scope = "project"  // ./DEEPORIN.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./DEEPORIN.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// DEEPORIN.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"DEEPORIN.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"DEEPORIN.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
