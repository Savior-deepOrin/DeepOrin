...
import (
	...
	"deeporin/internal/command"
	"deeporin/internal/config"
	"deeporin/internal/control"
	"deeporin/internal/event"
	"deeporin/internal/history"
	"deeporin/internal/hook"
	"deeporin/internal/installsource"
	"deeporin/internal/instruction"
	"deeporin/internal/jobs"
	"deeporin/internal/lsp"
	"deeporin/internal/memory"
	"deeporin/internal/netclient"
	"deeporin/internal/outputstyle"
	"deeporin/internal/permission"
	"deeporin/internal/plugin"
	"deeporin/internal/provider"
	"deeporin/internal/sandbox"
	"deeporin/internal/skill"
...
type Options struct {
	...
	// controller but are not persisted to deeporin.toml.
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w %q (configured: %s); note: defining [[providers]] replaces the built-in presets, so add a [[providers]] entry for it or use a configured name, or run `deeporin setup` to reconfigure", ErrUnknownModel, modelName, providerNames(cfg))
	}
	...
	if migErr != nil {
		sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "config migration from ~/.deeporin failed: " + migErr.Error()})
	...
	// Persistent memory (DEEPORIN.md / AGENTS.md hierarchy + auto-memory index)
	...
	if cfg.Codegraph.Enabled && !tokenEconomy {
		...
		default:
			sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelInfo,
				Text: "codegraph: not installed — run `deeporin codegraph install` to enable symbol-graph tools"})
		}
	}
	if !tokenEconomy {
	...
	// resolves "ask" to allow — preserving `deeporin run` autonomy — while deny
	...
	skillRunner := func(sctx context.Context, sk skill.Skill, task string, runOpts skill.SubagentRunOptions) (string, error) {
		...
		if subagentStore == nil || parentSession == "" {
			// Headless runs (e.g. `deeporin run`) have no persistent session to
	...
	// Custom slash commands (.deeporin/commands + user dir). Best-effort: a malformed
...
func migrateLegacySessionSources(sink event.Sink) {
	...
	if home, herr := os.UserHomeDir(); herr == nil {
		sources = append(sources, legacySource{
			dir:     filepath.Join(home, ".deeporin", "sessions"),
			label:   "~/.deeporin/sessions",
...
func rememberPermissionConfigPath(workspaceRoot string) string {
	...
	if workspaceRoot != "" {
		return filepath.Join(workspaceRoot, "deeporin.toml")
	}
	path := config.SourcePath()
	if path == "" {
		path = "deeporin.toml" // match Config.Save() fallback
	}
	return path
}
