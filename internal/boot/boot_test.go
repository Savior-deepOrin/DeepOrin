...
import (
	...
	"deeporin/internal/agent/testutil"
	"deeporin/internal/builtinmcp"
	"deeporin/internal/config"
	"deeporin/internal/event"
	"deeporin/internal/memory"
	"deeporin/internal/netclient"
	"deeporin/internal/plugin"
	"deeporin/internal/provider"
	"deeporin/internal/sandbox"
	"deeporin/internal/tool"
	"deeporin/internal/tool/builtin"

	// Blank import registers the provider kind the same way cmd/deeporin's main
...
// cache-first wiring: a project DEEPORIN.md is discovered at boot and folded
...
model = "x"
api_key_env = "DEEPORIN_TEST_KEY_UNSET"
`)
	writeFile(t, dir, "DEEPORIN.md", "Project rule: always run go vet before committing.")
	...
	if !strings.Contains(sys, "always run go vet before committing") {
		t.Fatalf("project DEEPORIN.md not folded into system message:\n%s", sys)
	}
	...
	if mem := ctrl.Memory(); mem == nil || len(mem.Docs) == 0 {
		t.Fatal("controller memory set is empty after discovering DEEPORIN.md")
	}
}
...
// `deeporin run`: a controller built via Build with NO SetSessionPath (exactly
...
api_key_env = "DEEPORIN_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".deeporin/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
[[plugins]]
name = "mockmcp"
command = "deeporin-missing-mockmcp"
`)
	writeFile(t, dir, ".deeporin/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
func TestBuildOmitsExcludedSkillRootsFromPromptAndRuntimeList(t *testing.T) {
	...
	writeFile(t, home, ".deeporin/skills/keep.md", "---\ndescription: keep\n---\nplaybook")
	writeFile(t, home, ".agents/skills/noisy.md", "---\ndescription: noisy\n---\nplaybook")
	writeFile(t, dir, "deeporin.toml", fmt.Sprintf(`
default_model = "test-model"
...
`)
	...
	// user-global DEEPORIN.md in the real config dir could append; the test
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	t.Chdir(cwd)
	writeFile(t, cwd, "deeporin.toml", `
[permissions]
allow = ["Bash(cwd*)"]
`)
	writeFile(t, workspace, "deeporin.toml", `
[permissions]
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "deeporin.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "deeporin.toml"))
...
`)
	...
	if !res.Saved || res.Path != filepath.Join(workspace, "deeporin.toml") {
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "deeporin.toml")); !os.IsNotExist(err) {
...
`)
	...
	cfg := config.LoadForEdit(filepath.Join(workspace, "deeporin.toml"))
...
// ~/.deeporin/config.json with no v1+ config present must be imported during
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	writeFile(t, proj, "deeporin.toml", "[codegraph]\nenabled = false\n")
	writeFile(t, filepath.Join(home, ".deeporin"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".deeporin", "sessions"), "chat-1.events.jsonl",
...
[[plugins]]
name = "legacy-eager"
command = "deeporin-missing-legacy-eager-mcp"
tier = "eager"
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "deeporin.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
[[plugins]]
name = "legacy-lazy"
command = "deeporin-missing-legacy-lazy-mcp"
tier = "lazy"
`)
...
func TestBuildDefaultsToNearestGitRoot(t *testing.T) {
	...
	writeFile(t, root, "deeporin.toml", `
...
[[plugins]]
name = "slowserver"
command = "deeporin-missing-slow-mcp-binary"
tier = "eager"
`)
