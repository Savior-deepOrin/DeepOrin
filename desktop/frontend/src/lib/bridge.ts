...
export interface AppBindings {
  ...
  ConnectVisionKey(apiKey: string): Promise<void>;
...
function bridgeBreadcrumb(method: string): string {
  ...
  if (/^(SaveProvider|AddOfficialProviderAccess|RemoveProviderAccess|DeleteProvider|SetProviderKey|ClearProviderKey|FetchProviderModels|ConnectKey|ConnectVisionKey)/.test(method))
...
function makeMockApp(): AppBindings {
  ...
  const globalWorkspaceRoot = "~/Library/Application Support/deeporin/global-workspace";
  let cwd = freshMock ? globalWorkspaceRoot : "~/projects/joyquant-db"; // mutable so PickWorkspace is visible in dev
  let workspaces = freshMock ? [] : ["~/projects/joyquant-db", "~/projects/joyquant-sys", "~/projects/deeporin", "~/projects/blade"];
  let mockEffort = "auto";
  ...
  const capSkills: SkillView[] = [
    ...
    { name: "init", description: "Scaffold a DEEPORIN.md for this repo", scope: "builtin", runAs: "inline", enabled: true },
  ];
  let capSkillRoots: SkillRootView[] = [
    { dir: "~/projects/deeporin/.deeporin/skills", scope: "project", priority: 1, status: "missing", configured: false, removable: true, skills: 0 },
    {
      dir: "~/my-skills",
      scope: "custom",
      priority: 5,
    ...
    },
    {
      ...
      skillItems: [
        ...
        { name: "init", description: "Scaffold a DEEPORIN.md for this repo", scope: "global", runAs: "inline" },
      ],
    },
  ];
  ...
  const settings: SettingsView = {
    ...
    agent: { temperature: 0.2, maxSteps: 0, plannerMaxSteps: 12, systemPrompt: "You are deepOrin, a coding agent.", coldResumePrune: true, reasoningLanguage: "auto" },
    bot: {
      enabled: !freshMock,
      model: "",
    ...
    telemetry: true,
    metrics: false,
    configPath: "~/projects/deeporin/deeporin.toml",
  ...
  if (freshMock) {
    settings.configPath = "~/.config/deeporin/config.toml";
  }
  ...
  return {
    ...
    async PickWorkspace() {
      ...
      return mockSwitchWorkspace(cwd.endsWith("another-project") ? "~/projects/deeporin" : "~/projects/another-project");
    },
    ...
    async SavePastedImage(_dataUrl: string) {
      return ".deeporin/attachments/mock.png";
    },
    async SaveClipboardImage() {
      return ".deeporin/attachments/mock-clipboard.png";
    },
    async SavePastedFile(name: string, _dataUrl: string) {
      return `.deeporin/attachments/mock-${name}`;
    },
    ...
    async AttachDropped(path: string) {
      ...
      return { kind: "attachment" as const, path: `.deeporin/attachments/mock-${name}` };
    },
    ...
    async Memory() {
      return {
        available: true,
        storeDir: "~/.config/deeporin/projects/-mock/memory",
        storeGlobalDir: "~/.config/deeporin/memory/global",
        docs: [
          {
            path: "DEEPORIN.md",
            scope: "project",
            body: "# deepOrin project memory\n\nMock doc shown in the browser dev seam.\n\n## Notes\n\n- prefers concise replies",
          },
          {
            path: "~/.config/deeporin/DEEPORIN.md",
            scope: "user",
        ...
        archives: [
          {
            ...
            path: "~/.config/deeporin/projects/-mock/memory/.archive/20260612-021500.000-old-plan.md",
            archivedAt: "2026-06-12T02:15:00Z",
          },
        ],
        scopes: [
          { scope: "user", path: "~/.config/deeporin/DEEPORIN.md" },
          { scope: "project", path: "DEEPORIN.md" },
          { scope: "local", path: "DEEPORIN.local.md" },
        ],
      };
    },
    async MemorySuggestions() {
      return {
        ...
        skills: [
          {
            id: "skill-deeporin-pr-followup",
            name: "deeporin-pr-followup",
            description: "Review or update a deepOrin GitHub PR, address feedback, verify, and publish safely.",
            scope: "project",
            body: "# deepOrin PR Followup\n\nUse this skill for repeated deepOrin PR work.\n\n## Workflow\n\n1. Confirm branch and PR state.\n2. Inspect the diff.\n3. Fix actionable feedback.\n4. Verify and update the PR.\n",
    ...
    async AcceptSkillSuggestion(suggestion: SkillSuggestion) {
      ...
      return `.deeporin/skills/${suggestion.name}/SKILL.md`;
    },
    ...
    async Remember(_scope: string, _note: string) {
      ...
      return `${_scope} DEEPORIN.md (mock): ${_note}`;
    },
      ...
      settings.permissions[k] = settings.permissions[k].filter((r) => r !== rule);
        ...
        async StartBotConnectionInstall(provider: string, domain: string) {
          ...
          return {
            ...
            url: "https://example.com/deeporin-bot-qr",
    ...
    async OpenDownloadPage() {
      if (typeof window !== "undefined") {
        window.open("https://github.com/esengine/deeporin/releases/latest", "_blank", "noopener");
      }
    },
    ...
    async ConnectVisionKey(apiKey: string) {
