...
export function OnboardingOverlay({ onComplete }: { onComplete: () => void }) {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      // Save vision key if provided (optional)
      const vKey = visionValue.trim();
      if (vKey) {
        try { await app.ConnectVisionKey(vKey); } catch (_) { /* optional */ }
      }
      onComplete();
    } catch (e) {
  ...
  }, [t, value, visionValue, onComplete]);

  return (
    <div className="onboarding">
      <div className="onboarding__card">
        <img src={logo} className="onboarding__logo" alt="deepOrin" draggable={false} />
        ...
        <label className="onboarding__label" htmlFor="onboarding-vision-key" style={{ marginTop: "16px" }}>
          {t("onboarding.visionLabel")}
        </label>
        <input
          id="onboarding-vision-key"
          className="onboarding__input"
          type="password"
          autoComplete="off"
          spellCheck={false}
