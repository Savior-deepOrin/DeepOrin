...
func TestProjectHooksSettingsUseActiveWorkspaceRootAndTrust(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(project, ".deeporin", "settings.json")); err != nil {
...
func TestSaveHooksSettingsForRootUsesDisplayedProjectRoot(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(projectA, ".deeporin", "settings.json")); err != nil {
		t.Fatalf("displayed project root settings missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(projectB, ".deeporin", "settings.json")); err == nil {
