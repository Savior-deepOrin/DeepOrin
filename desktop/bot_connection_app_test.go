...
func TestRememberBotConnectionRemoteStoresStableScope(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/tmp/deeporin-project",
	}, nil); err != nil {
	...
	if projectMapping.Scope != "project" || projectMapping.WorkspaceRoot != "/tmp/deeporin-project" || projectMapping.RemoteID != "wxid_project" {
