# deepOrin Desktop (Wails shell)

A native desktop window around the deepOrin Go kernel. The same
...
`desktop/` is its own Go module (`module deeporin/desktop`, `replace deeporin =>
...
`deeporin/` so it can still import the `deeporin/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/deepOrin(.app/.exe)
```
...
- **macOS** — open `deepOrin-darwin-universal.dmg` and drag deepOrin into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/deepOrin.app
  ```
...
```sh
minisign -Vm deepOrin-darwin-arm64.zip \
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `DEEPORIN_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  off.
...
The desktop app sends one anonymous ping per launch to `crash.deeporin.io`:
