...
// by substring (file names embed the key, e.g. deepOrin-darwin-arm64.zip), so the
...
// (deeporin.key) and the public key (deeporin.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "deeporin.key")
	pubPath := filepath.Join(dir, "deeporin.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "deepOrin desktop release")
		out := f + ".minisig"
