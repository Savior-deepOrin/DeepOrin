...
import (
	...
	"deeporin/desktop/internal/update"
)
...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "deepOrin-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	dir := t.TempDir()
	names := []string{
		"deepOrin-darwin-arm64.zip",
		"deepOrin-darwin-amd64.zip",
		"deepOrin-windows-amd64-installer.exe",
		"deepOrin-windows-amd64.zip", // portable download, not the updater channel
		"deepOrin-windows-arm64-installer.exe",
		"deepOrin-windows-arm64.zip", // portable download, not the updater channel
		"deepOrin-linux-amd64.tar.gz",
		"deepOrin-linux-amd64.deb",            // human download, not the updater channel
		"deepOrin-linux-amd64.tar.gz.minisig", // must be skipped
	...
	t.Setenv("GITHUB_REPOSITORY", "Savior-deepOrin/deeporin")
	...
	wantURL := "https://github.com/Savior-deepOrin/deeporin/releases/download/desktop-v1.2.0/deepOrin-windows-amd64-installer.exe"
	if win.URL != wantURL {
	...
	if !strings.HasSuffix(arm.URL, "/deepOrin-windows-arm64-installer.exe") {
	...
	if !strings.HasSuffix(lin.URL, "/deepOrin-linux-amd64.tar.gz") {
