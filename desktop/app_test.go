...
func TestSettingsUsesUserDesktopPreferencesNotProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "deeporin.toml"), []byte(`
[desktop]
language = "zh"
theme = "light"
theme_style = "glacier"
...
func TestDeleteProviderMigratesConfigAndOpenTabs(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("DEEPORIN_TEST_KEY", "sk-test")

	cfg := config.Default()
	cfg.DefaultModel = "prov-a/model-a2"
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", Models: []string{"model-a1", "model-a2"}, APIKeyEnv: "DEEPORIN_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "DEEPORIN_TEST_KEY"},
	}
...
func TestDeleteProviderRejectsRunningAffectedTab(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", APIKeyEnv: "DEEPORIN_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "DEEPORIN_TEST_KEY"},
	}
	if err := cfg.SaveTo(config.UserConfigPath()); err != nil {
		t.Fatalf("save config: %v", err)
...
func TestSubmitToTabHistoryDisplaysRawInputAfterMemoryCompose(t *testing.T) {
	...
	ctrl.QueueMemory(`Saved memory "deeporin-contributions": contribution count updated`)
...
func TestForkCreatesActiveTabWithoutSwitchingSourceController(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(workspace, "deeporin.toml"), []byte("[codegraph]\nenabled = false\n"), 0o644); err != nil {
...
func TestCapabilitiesShowsDefaultMCPAsInitializingNotDisabled(t *testing.T) {
	...
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "deeporin.toml"), []byte(`
[codegraph]
enabled = false
...
`), 0o644); err != nil {
	...
	projectCfg := config.LoadForEdit(filepath.Join(dir, "deeporin.toml"))
...
`), 0o644); err != nil {
	...
	if err := app.UpdateMCPServer("broken", MCPServerInput{
		Name:      "broken",
		Transport: "stdio",
		Command:   "deeporin-missing-mcp-binary",
	}); err != nil {
	...
	if got := cfg.Plugins[0].Command; got != "deeporin-missing-mcp-binary" {
	...
	for _, s := range view.Servers {
		if s.Name == "broken" {
			...
			if s.Command != "deeporin-missing-mcp-binary" || s.Tier != "background" {
...
func TestSetMCPServerTierEnablesCodegraphAndIgnoresLegacyTier(t *testing.T) {
	...
	t.Setenv("DEEPORIN_CACHE_DIR", robustTempDir(t)) // isolate the codegraph bundle cache so Resolve fails deterministically
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "deeporin.toml"), []byte(`
[codegraph]
